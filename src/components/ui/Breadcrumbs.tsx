...
export default function Breadcrumbs({
  ...
  return (
    <nav
      ...
      className="min-w-0 text-xs text-navy-500 sm:text-sm"
    >
      {/* Mobile */}
        ...
        {items.map((item, index) => (
          ...
          >
            {item.href ? (
              <Link
                href={item.href}
                className="whitespace-nowrap transition-colors hover:text-navy-800"
              >
                {item.name}
              </Link>
            ) : (
              <span className="max-w-[260px] truncate text-navy-800 lg:max-w-[420px]">
                {item.name}
              </span>
            )}
